...
for object in mitreData['objects']:
    ...
    if object['type'] =='attack-pattern':
        if 'external_references' in object:
            for refernce in object['external_references']:
                if 'external_id' in refernce:
                    if ((refernce['external_id'].startswith("T"))):
                        if 'kill_chain_phases' in object:
                            for tactic in object['kill_chain_phases']:
                                tactics.append(tactic['phase_name'])
                        technique = refernce['external_id']
                        name = object['name']
                        url = refernce['url']
                        ...
                        if 'x_mitre_deprecated' in object:
                            ...
                            filtered_object = {'tactcs': str(tactics), 'technique': technique, 'name': name, 'url': url, 'deprecated': deprecated}
                            mitreMapped[technique] = filtered_object
                        else:
                            filtered_object = {'tactcs': str(tactics), 'technique': technique, 'name': name, 'url': url, 'deprecated': "False"}
                            mitreMapped[technique] = filtered_object                    
...
for file in alert_data:
    for line in alert_data[file]:
        ...
        # Check to ensure MITR tactics exist
        ...
        # Check to see if the MITRE technique is deprecated
        try:
